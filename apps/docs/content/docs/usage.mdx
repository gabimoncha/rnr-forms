---
title: Usage
description: How to use the Universal Forms
---

## Using the template

- The example components are found in the `packages/rnr-forms` folder.
...
- The Reusables UI are installed in `packages/rnr-forms/reusables`.

## Updating the docs

<Callout>
  **Notes: ** When changing the `packages/rnr-forms` folder name, you need to update the
...
- `packages/rnr-forms/package.json` to update the package name
- `packages/rnr-forms/src/components/ui/*` to create your components
- `packages/rnr-forms/src/examples/*` to update the example components
- `packages/rnr-forms/src/blocks/*` to update the examples blocks
